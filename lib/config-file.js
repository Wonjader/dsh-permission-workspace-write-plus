...
export async function isAllowedExtra(targetPath, workspaceRoot, options) {
  ...
  const allowWriteDirs = Array.isArray(options?.allowWriteDirs) ? options.allowWriteDirs : []
  ...
  for (const dir of allowWriteDirs) {
    if (typeof dir !== 'string' || dir.trim().length === 0) continue
    if (await isPathUnder(targetPath, dir.trim()).catch(() => false)) return true
  }
  return false
}
