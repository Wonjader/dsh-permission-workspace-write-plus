...
export const SETTINGS_NS = 'workspace-write-plus'

/**
 * Compatibility shim for the two dsh-settings consumer APIs:
 *  - older harness: named export `installSettingsSection(ctx, ns, schema, entry, hooks)`
 *  - newer harness: provider method `ctx.settings.installSection(owner, ns, schema, entry, hooks)`
 * The namespace import avoids a static ESM link error when one of the named
 * exports is absent in the running harness.
 */
function installSettingsSectionCompat(ctx, ns, schema, entry, hooks) {
  if (typeof settingsApi.installSettingsSection === 'function') {
    const brandedNs = typeof settingsApi.settingsNamespace === 'function'
      ? settingsApi.settingsNamespace(ns)
      : ns
    settingsApi.installSettingsSection(ctx, brandedNs, schema, entry, hooks)
    return
  }
  ctx.inject(['settings'], (settingsCtx) => {
    const provider = settingsCtx.settings
    if (provider === undefined || typeof provider.installSection !== 'function') {
      throw new Error('dsh-settings: no compatible settings registration API (installSettingsSection or installSection)')
    }
...
export const SettingsSchema = z.object({
  ...
  // User-configured extra writable directories. Each entry is an absolute
  // directory path; empty by default.
  allowWriteDirs: z.array(z.string()).default([]),
})
...
export function resolveSettings(stored, config = {}) {
  ...
  const pickStringArray = (key) => {
    const normalize = (value) => Array.isArray(value)
      ...
      : undefined
    const storedValue = normalize(stored?.[key])
    if (storedValue !== undefined) return storedValue
    const configValue = normalize(config?.[key])
  ...
  return {
    ...
    allowWriteDirs: pickStringArray('allowWriteDirs'),
  }
}
...
export function installSettings(ctx, entryConfig, onResolved) {
  ...
  installSettingsSectionCompat(ctx, SETTINGS_NS, SettingsSchema, entryConfig ?? DEFAULT_SETTINGS, {
...
function pickSwitches(config) {
  ...
  for (const key of Object.keys(DEFAULT_SETTINGS)) {
    ...
    else if (key === 'allowWriteDirs' && Array.isArray(config[key])) out[key] = config[key]
  }
